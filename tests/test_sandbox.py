...
from config import CostsConfig, SizingConfig  # noqa: E402
...
def sandbox(tmp_path) -> Sandbox:
    ...
    return Sandbox(
        conn, run_id="test-run", mode="backtest",
...
def test_no_pyramiding(sandbox):
    ...
    skip = sandbox.conn.execute(
        "SELECT reason FROM trade_log WHERE action = 'SKIP'"
    ).fetchone()
